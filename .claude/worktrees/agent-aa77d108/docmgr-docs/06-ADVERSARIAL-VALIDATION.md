# Adversarial Validation Plan: `docmgr` v1.0
...
**Steps:**
1. Init store, start `docmgr open`
...
3. In a tight loop, overwrite `rapid.md` 20 times in 1 second:
   ...
   ```
4. Wait 3 seconds
5. `docmgr log rapid.md`
6. `docmgr show rapid.md v<latest>`
...
**Expected:**
- `docmgr` captures at least 1 version (the state at poll time), possibly more
...
2. Write a script that in a tight loop: creates `ephemeral.md`, immediately deletes it
   ...
   ```
3. Run for 10 seconds with `docmgr open` active
4. Stop the script
5. Check `docmgr status` and `docmgr log`
...
3. Simultaneously (within the same 50ms):
   ...
   ```bash
   cat > .docmgr/locks/agent-lock.toml << EOF
...
2. Start `docmgr open` (writes manifest on changes)
3. In another terminal, repeatedly read manifest:
   ```bash
   while true; do cat .docmgr/manifest.toml | wc -l; sleep 0.05; done
   ```
...
3. Start a slow write that takes >1 second:
   ...
   "
   ```
4. Check if `docmgr` commits a partial file

**What can go wrong:**
- Poll detects the file has changed (mtime updated), reads and commits it while it's still being written

**Expected:**
- This is a known limitation of stat-based polling. `docmgr` may commit a partial file.
...
2. Start `docmgr open --agent=evil-agent`
...
2. Start `docmgr open --agent=clever-agent`
...
**What can go wrong:**
- `docmgr` reverts the file, but the writer immediately overwrites the revert
- `docmgr` enters an infinite revert loop
- CPU spins at 100% on continuous revert/overwrite cycle

**Expected:**
- While the writer is active, `docmgr` continuously reverts (this is correct behavior)
...
3. `docmgr unlock ref.md --for=agent --duration=1` (1 minute override)
...
2. Start `docmgr open --agent=fast-agent`
...
2. Script that modifies each file 200 times (1000 total commits):
   ...
   for round in $(seq 1 200); do
     ...
     sleep 1.5  # let docmgr commit each round
   done
   ```
3. `docmgr log --limit=50` — measure time
4. `docmgr log a.md` — measure time
5. `docmgr diff a.md v1 v200` — measure time
6. `docmgr info a.md` — check version count
...
**Expected:**
- On restart, `docmgr` detects stale `index.lock` and removes it (or `git2` handles it)
...
4. `docmgr ls`
5. `docmgr info a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/deep.md`
...
2. Create a `.md` file with binary content:
   ...
   ```
3. Wait for detection
4. `docmgr diff binary.md`
5. `docmgr info binary.md`
...
2. `touch empty.md`
3. Wait for detection
4. `docmgr info empty.md`
5. Modify it: `echo "now has content" > empty.md`
6. `docmgr diff empty.md`
...
### DC-1: DOCMGR.md Consistency Under Rapid Changes
...
4. Wait 3 seconds
5. Read `DOCMGR.md`
6. Compare its listed files against `docmgr ls`

**Expected:**
- `DOCMGR.md` matches the current state of the manifest exactly
- No stale entries (deleted files still listed)
- No missing entries (new files not listed)
- File types in `DOCMGR.md` match manifest types
...
### DC-4: Agent Reads DOCMGR.md While It's Being Regenerated

**Steps:**
1. Init store
2. Start `docmgr open`
3. In a loop, read `DOCMGR.md` rapidly while also modifying files:
   ```bash
   while true; do cat DOCMGR.md | wc -l; sleep 0.05; done &
   for i in $(seq 1 20); do echo "new" > "file-$i.md"; sleep 0.3; done
   ```

**Expected:**
- Reader always sees a complete `DOCMGR.md` (never a partial write)
- `DOCMGR.md` should be written atomically (write-tmp-rename) like the manifest
...
**Expected:**
- `docmgr` detects it can't read the file
...
2. Start `docmgr open /tmp/store-a`
3. `mv /tmp/store-a /tmp/store-b`

**Expected:**
- `docmgr` detects the directory is gone on next poll
...
- Behavior is well-defined: either `docmgr` respects the new ignore (stops tracking) or continues tracking (git tracks files already committed even if later ignored)
- No crash either way
...
3. After all operations, run `docmgr repair`
...
2. For each version 1-20, run `docmgr show <file> v<N>`
...
4. `docmgr log new-name.md`
5. `docmgr show new-name.md v2` (from before the rename)
...
- `docmgr info new-name.md` shows version v9 (or whatever the correct count is including the rename)

### DI-4: Restore Doesn't Corrupt Subsequent Versions

**Steps:**
1. Create `doc.md` with "v1", modify to "v2", modify to "v3"
2. `docmgr restore doc.md v1` (now at v4 with v1 content)
3. Modify to "v5 new content"
4. `docmgr show doc.md v2` — should still be "v2"
5. `docmgr show doc.md v3` — should still be "v3"
6. `docmgr show doc.md v4` — should be "v1" (restored)
7. `docmgr show doc.md v5` — should be "v5 new content"
...
| DC-1 | DOCMGR.md Consistency | PASS | DOCMGR.md lists all manifest entries after all change rounds |
...
- **DC-4 / non-atomic DOCMGR.md**: `std::fs::write` replaced with tmp→rename atomic pattern.
