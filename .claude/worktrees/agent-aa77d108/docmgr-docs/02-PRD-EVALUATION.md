...
**One concern:** The phrase "local interactive server" in the summary is misleading. `docmgr` is not a server — it's a TUI application that runs a polling loop. The word "server" implies network listeners, HTTP endpoints, etc. Recommend changing to "local interactive terminal application."
...
Three personas are clear. However, the third persona ("Agent framework developers") implies `docmgr` exposes a programmatic API or protocol that agents can integrate with. The current PRD only offers a file-based lock mechanism (Section 7.2) and the agents write to the filesystem directly. **This persona's need ("standardized document store that agents can read from and write to") is mostly unmet in V1** — agents just write files and `docmgr` reacts. That's fine, but the persona description oversells V1's agent integration.
...
**Minor gap:** What happens if a snapshot file is missing but referenced by a version entry? This is a corruption scenario. `docmgr repair` should handle it, but the expected behavior isn't specified. Recommend: "If a snapshot is missing, the version entry is marked as `corrupted` and the user is warned. The document's current on-disk content is always available as the latest version regardless of snapshot state."
...
**Gap in 5.1.2 (Document Registration):** "Polling detection — `docmgr` detects new `.md` files during its poll cycle." But the poller (Section 5.2.1) says it calls `stat()` on "every tracked file" and also "scans for new `.md` files." The scan-for-new-files part is a recursive directory walk. For a store with deep nesting, this could be slow.
...
**Recommendation:** "The poller recursively scans all subdirectories of the store root, excluding `.docmgr/` and paths matching the ignore file. There is no depth limit. For V1, stores with >1000 files may experience slower poll cycles; this is acceptable."
...
**Gap:** How does `docmgr` decide WHICH log document to append to? If there are multiple `log`-type documents, does it create a new one? Append to the most recent? Use a naming convention?

**Recommendation:** "When synthesizing a log entry, `docmgr` appends to the log document tagged with the current agent's session ID (from the agent-lock file). If no such document exists, `docmgr` creates a new log document named `logs/<agent-name>-<session-id>.md`."
...
**Gap:** The agent-lock file specifies a `pid` field and `docmgr` removes stale locks where the PID no longer exists. How does `docmgr` check if a PID exists portably? On Linux: check `/proc/<pid>`. On macOS: `kill(pid, 0)`. The `kill` signal-zero approach works on both. Specify: "Stale lock detection uses `kill(pid, 0)` (signal zero) — if the process doesn't exist, the lock is considered stale."
...
|-------------|-----------------|
| Stat-based polling detects file create | Create a `.md` file in the store. Within 2 poll cycles, `docmgr status` shows it as tracked. |
...
The current first-run experience requires: (1) install binary, (2) run `docmgr init`, (3) optionally run `docmgr model download`, (4) run `docmgr open`. If the user skips step 3, they get degraded functionality with a warning message. This multi-step setup is a usability risk for adoption.

**Recommendation:** Add a risk entry: "First-run friction — Likelihood: Medium, Impact: Medium. Mitigation: `docmgr open <path>` should auto-init if no store exists, reducing the required steps to one command."
...
**Recommendation:** Add a risk entry and a mitigation: "`docmgr repair` can rebuild from filesystem state."
...
1. User installs `docmgr` binary
2. Has an existing project folder with 5 markdown planning docs
3. Runs `docmgr open ./my-project`
4. `docmgr` detects no `.docmgr/` — asks "Initialize a new store? [y/N]"
...
9. Next poll cycle: `docmgr` detects the new file, shows it in green in the tree, adds a changelog entry.
...
2. Runs `docmgr open ./my-project --agent=claude-code`
...
| Condition | Message |
|-----------|---------|
| Store not initialized | "Not a docmgr store. Run `docmgr init <path>` to create one." |
| Corrupted manifest | "Manifest is corrupted. Run `docmgr repair` to rebuild from filesystem." |
| LLM not configured | "LLM not configured. NL commands disabled. Run `docmgr model download` to set up." |
| Concurrent instance | "Another docmgr instance is running (PID: 12345). Opening in read-only mode." |
| File not tracked | "`notes.md` is not tracked. Run `docmgr add scratch notes.md` to track it." |
| Snapshot missing | "Warning: Snapshot for `prd.md` v2 is missing. Version may be unrecoverable." |

### Issue C: Relationship Between `docmgr init` and `docmgr open`

The PRD has both commands but the boundary is fuzzy:

- `docmgr init /path` creates a store.
- `docmgr open /path` launches the TUI.
- If the user runs `docmgr open` on a non-store directory, what happens? Error? Auto-init?

The "Missing risk" in Section 12 feedback suggested `docmgr open` should auto-init. The PRD should take a position on this.

**Recommendation:** `docmgr open /path` on a non-store directory should prompt: "No store found at /path. Initialize one? [y/N]". This collapses the two-step init+open into one command for the common case.

### Issue D: No Specification for the `docmgr status` Display Format

`docmgr status` is referenced in both CLI and TUI contexts, but the output format is never shown. Implementers need to know what it looks like:
...
**Recommendation:** Either remove tags from V1 (simplify) or add a brief section: "Tags are free-form strings. `docmgr tag <file> <tag>` adds a tag. `docmgr ls --tag=<tag>` filters by tag. Tags have no system-level meaning; they are for user organization."
...
8. Add `docmgr status` output format (Issue D)
...
11. Specify `docmgr open` behavior on non-store directory (Issue C)
