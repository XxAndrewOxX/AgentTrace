...
pub fn load_pending_updates(store_root: &Path) -> Result<Vec<ContextUpdate>> {
    let path = store_root.join(".docmgr").join("context_updates.jsonl");
    if !path.exists() {
        return Ok(Vec::new());
    }
...
pub fn synthesize_no_llm(store_root: &Path, manifest: &Manifest) -> Result<String> {
    ...
    out.push_str(&format!(
        "*Auto-generated by docmgr on {}. Use `docmgr context refresh` to update.*\n\n",
...
mod tests {
    ...
    fn test_pending_updates_loaded() {
        ...
        std::fs::write(
            root.join(".docmgr").join("context_updates.jsonl"),
            entry.to_string() + "\n",
        ).unwrap();
