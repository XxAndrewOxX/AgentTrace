...
pub fn generate_and_commit(store_root: &Path, manifest: &Manifest, git: &GitStore) -> Result<()> {
    ...
    let info = CommitInfo {
        ...
        actor: Actor::System,
        summary: "update DOCMGR.md index".into(),
        agent_name: None,
        session_id: None,
    };
    git.commit(&info)?;
...
pub fn generate(_store_root: &Path, manifest: &Manifest) -> String {
    ...
    let mut out = String::from("# DOCMGR.md — Agent Discovery Index\n\n");
    out.push_str(
        "> This file is auto-generated by `docmgr`. Do not edit manually.\n\
...
mod tests {
    ...
    fn test_generate_empty_store() {
        ...
        assert!(content.contains("DOCMGR.md"));
    ...
    fn test_generate_and_commit() {
        ...
        // Need prd.md to exist to stage it (already in DOCMGR generation we only write DOCMGR.md)
        generate_and_commit(&root, &manifest, &git).unwrap();
        assert!(root.join("DOCMGR.md").exists());
    }
