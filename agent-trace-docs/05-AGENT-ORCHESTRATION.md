# Agent Orchestration Guide: `agent-trace` Build

**Purpose:** Instructions for using Claude Code with parallel sub-agents in tmux to implement `agent-trace` from the PRD and implementation plan.
...
impl Actor {
    pub fn git_author(&self) -> (&str, &str) {
        match self {
            Actor::User => ("User", "user@agent-trace"),
            Actor::Agent(name) => {
                // Returns ("Agent: claude-code", "agent@agent-trace")
                // Caller must format the name into the string
                ("Agent", "agent@agent-trace")
            }
            Actor::System => ("agent-trace", "system@agent-trace"),
            Actor::Llm => ("User via LLM", "llm@agent-trace"),
        }
    }
...
fn main() {
    println!("agent-trace - not yet implemented");
}
EOF

# Initialize git for the project itself (NOT the agent-trace store git — this is the source code repo)
git init
git add -A
...
- GitStore::init(store_root) — create repo in .agent-trace/repo/
...
- Overrides struct: load/save from .agent-trace/locks/overrides.toml, add, check, prune expired
...
tmux new-session -d -s agent-trace-build -n orchestrator

# Create panes for each agent
tmux split-window -h -t agent-trace-build:orchestrator
tmux split-window -v -t agent-trace-build:orchestrator.0
tmux split-window -v -t agent-trace-build:orchestrator.1
tmux split-window -v -t agent-trace-build:orchestrator.2
tmux split-window -v -t agent-trace-build:orchestrator.3
...
# Attach
tmux attach -t agent-trace-build
```

In each pane, `cd agent-trace && claude` and paste the corresponding agent prompt.
...
**Files owned:** `src/poll.rs`, `src/context.rs`, `src/log_synth.rs`, `src/agent-trace_md.rs`, `src/tui/*.rs`
...
- Stream 6: Poll loop, change processor, agent attribution, log synthesis, AGENT-TRACE.md generation, instance locking
...
- src/agent-trace_md.rs (Task 6.5 — AGENT-TRACE.md generation)
- src/tui/mod.rs
...
- Also update src/main.rs to wire everything together for `agent-trace open`
