# E2E Validation Plan: `agent-trace` v1.0
...
2. Run `agent-trace init /tmp/test-empty`
3. Run `agent-trace status`
4. Run `agent-trace open`
5. In TUI, type `help`
6. Press `q` to exit

**Expected:**
- `.agent-trace/` created with: `config.toml`, `manifest.toml`, `repo/` (valid git repo), `locks/`, `context_updates.jsonl`, `command_history.txt`
- `.gitignore` at store root with default patterns
- `AGENT-TRACE.md` at store root with "0 documents" in stats
...
2. Run `agent-trace init /tmp/test-pop --scan`
3. Run `agent-trace ls`
4. Run `agent-trace info <one-of-the-files>`

**Expected:**
- All 5 `.md` files registered in manifest as type `scratch`
- The `.txt` and `.json` files are NOT tracked
- Git repo has an initial commit containing all 5 `.md` files + manifest + AGENT-TRACE.md
- `ls` shows 5 files with `[S]` type indicators
- `info` shows version v1, created_at = init timestamp, created_by = user
- `AGENT-TRACE.md` lists all 5 files under "Scratch"
...
3. `agent-trace replace "PostgreSQL" "MySQL" --type=plan --dry-run`
4. `agent-trace replace "PostgreSQL" "MySQL" --type=plan`
5. Confirm when prompted
6. Check file contents
7. `agent-trace log --limit=1`
...
4. `agent-trace context update "We decided to switch to Rust"`
5. `agent-trace context updates`
6. `agent-trace context refresh`
7. `agent-trace context show`
...
**Steps:** Execute every non-interactive command in sequence without ever launching `agent-trace open`:

1. `agent-trace init /tmp/test-cli --scan` (with 3 .md files)
2. `agent-trace status`
3. `agent-trace ls`
4. `agent-trace ls --json`
5. `agent-trace info <file>`
6. `agent-trace add plan new-plan.md` (after creating the file)
7. `agent-trace reclassify <file> reference`
8. `agent-trace log`
9. `agent-trace log <file>`
10. Modify a file externally, then `agent-trace status`
11. `agent-trace diff <file>`
12. `agent-trace show <file> v1`
13. `agent-trace restore <file> v1`
14. `agent-trace replace "foo" "bar" --dry-run`
15. `agent-trace context refresh`
16. `agent-trace context update "test"`
17. `agent-trace context updates`
18. `agent-trace context show`
19. `agent-trace unlock <file> --for=agent --duration=1`
20. `agent-trace violations`
21. `agent-trace untrack <file>`
22. `agent-trace rm <file>`
23. `agent-trace repair`
...
2. Start `agent-trace open` in background
3. Externally create `.agent-trace/locks/agent-lock.toml` with agent name "test-agent", valid PID
...
- AGENT-TRACE.md updated with new file
...
**Steps:**
1. Init store with 2 plan files
2. Start `agent-trace open --agent=claude-code`
...
**Steps:**
1. Init store, create a plan, run `agent-trace context refresh` to generate `context.md`
...
- `agent-trace violations` lists this violation
...
### AI-7: AGENT-TRACE.md as Agent Discovery

**Steps:**
1. Init store with 5 documents of various types
2. Read `AGENT-TRACE.md`
...
**Steps:**
1. Create `.agent-trace/locks/agent-lock.toml` with a PID of a process that doesn't exist
...
- `agent-trace` operates in normal user mode (no agent attribution)
...
4. `agent-trace unlock api-schema.md --for=agent --duration=2`
...
2. `agent-trace violations` → shows all 5
3. `agent-trace violations --limit=3` → shows most recent 3
4. Check git log → all 5 have `[agent-trace] violation` commits
...
**Steps:**
1. Init store
2. Create `.agent-trace/.manifest.toml.tmp` (simulating an interrupted atomic write)
3. Start `agent-trace`
...
**Steps:**
1. Init store with 5 documents
2. Delete a random file from `.agent-trace/repo/objects/`
3. Run `agent-trace status`

**Expected:**
- `agent-trace` detects the git error and reports it clearly
- Suggests: "Git repository may be corrupted. Run `agent-trace repair` to attempt recovery."
- `repair` can rebuild from current files on disk (loses history but preserves current state)

### FR-5: Graceful Shutdown Under Load

**Steps:**
1. Start `agent-trace open`
...
2. Measure time from `agent-trace open` to TUI first frame (without LLM)
...
2. `agent-trace log --limit=50` — measure time
3. `agent-trace log <file>` on a file with 200 versions — measure time
...
**Steps:**
1. Start `agent-trace open` on a store with 200 documents
...
| Agent | AI-7 AGENT-TRACE.md Discovery | PASS | Fixed `add` to regenerate AGENT-TRACE.md |
