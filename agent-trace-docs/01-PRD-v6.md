# Product Requirements Document: `agent-trace` — Agent Document Manager

**Version:** 0.5.0-draft  
**Date:** 2026-04-02  
**Status:** Draft — Pending User Review  
**Revision Notes:** v5 — Two major changes: (1) Context is now system-synthesized, not user-authored. `agent-trace` reads all plans and reference docs and generates `context.md` automatically. Users steer context via `agent-trace context update` commands. (2) Added agent integration protocol via `AGENT-TRACE.md` — a generated index file at the store root that any agent can read for zero-cost discovery. The folder IS the API.

---

## 1. Executive Summary

`agent-trace` is a lightweight Rust CLI binary that provides **document-as-first-class-object management** for AI agent workflows. It treats every document — plans, logs, context files — as a tracked, versioned, semantically-typed entity within a local folder ("document store"). It combines three core capabilities:
...
### What `agent-trace` Solves

`agent-trace` gives users a **single pane of glass** for all agent-related documents: what exists, what changed, what agents did, and a natural language interface to steer the entire document corpus.
...
- **System-generated documents** — `context.md` (synthesized project overview), log files, `AGENT-TRACE.md` (agent index)
- **Hidden metadata directory** — `.agent-trace/` at the root of the store
...
| `context` | **System-synthesized** project overview. A derived document that `agent-trace` builds and maintains by reading all other documents in the store. External agents read this to understand the current project state. | Auto-generated project summary, current status, key decisions, open questions |
...
| `context` | **`agent-trace` only** | **`agent-trace` only** (user steers via CLI) | All | Context is synthesized by the system from all documents in the store. It's a derived view, not a source document. This ensures context always reflects reality. The user steers context indirectly (by modifying plans/reference) or directly via `agent-trace context update` commands that feed the next synthesis. |
| `log` | **`agent-trace` only** | **`agent-trace` only** | All | Logs are the system's independent audit trail. `agent-trace` synthesizes what agents did by watching their actual file operations. If agents could write their own logs, the logs wouldn't be trustworthy — agents might be inconsistent, verbose, or omit failures. The system observes and records objectively. |
...
Context documents are **automatically generated and maintained** by `agent-trace`. They are derived artifacts, not source documents.

**How context synthesis works:**

1. **Trigger:** Context is re-synthesized when:
   - The store is initialized (`agent-trace init --scan`)
   - Any `plan` or `reference` document is created, modified, or deleted
   - The user explicitly requests it: `agent-trace context refresh`
   - The user injects a specific update: `agent-trace context update "We decided to use PostgreSQL instead of SQLite"`

2. **Synthesis process (LLM required):**
   - `agent-trace` reads the current content of all `plan` and `reference` documents (truncated to fit context window)
   - Reads the current `context.md` (if it exists)
   - Reads any user-provided context updates (from the `agent-trace context update` command)
...
4. **User steering:** The user influences context in three ways:
   ...
   - **Via update command:** `agent-trace context update "The API launch date moved to July 15"` — queues a statement for the next synthesis.
   - **Via refresh command:** `agent-trace context refresh` — forces a full re-synthesis.

5. **Context update queue:** User updates are stored in `.agent-trace/context_updates.jsonl` until incorporated:
   ```json
...
Write permissions are enforced at the **change processing layer**, not the filesystem layer. `agent-trace` cannot prevent an agent from physically writing to any file (it's just a folder on disk). Instead:
...
2. **On detection of a write to a `log` or `context` document by the user:**
   ...
   - For `context`: Triggers: "Context is system-synthesized. Use `agent-trace context update \"...\"` to update. Overwrite directly? [y/N]". If confirmed, applied with `actor: "user-override"` (will be overwritten on next synthesis).
...
agent-trace unlock context.md --for=agent --duration=10m

# Permanently change a document's permissions (by reclassifying)
agent-trace reclassify context.md plan    # Now agents can write to it
```

The `unlock` command creates a temporary override entry in `.agent-trace/locks/overrides.toml`:
...
When the LLM is available, `agent-trace` auto-classifies new documents. Without the LLM, new documents default to `scratch` and the user reclassifies manually. **Agents cannot assign types** — type assignment is done by `agent-trace` (via LLM) or by the user (via CLI). This prevents an agent from creating a file and self-classifying it as `context` to bypass write protections.
...
*Context is system-synthesized. Users influence it via `agent-trace context update "..."` or indirectly by modifying plans/reference docs. Direct edits to `context.md` trigger a confirmation prompt and are tagged as `user-override`.
...
### 4.3 Hidden Metadata (`.agent-trace/` directory)
...
│   └── ...
├── locks/
│   ├── instance.lock        # Prevents concurrent agent-trace instances
...
- ~~`.agent-trace/ignore`~~ → `.gitignore` at the store root (standard git behavior)

**Store identification:** A folder is a valid `agent-trace` store if and only if `.agent-trace/config.toml` exists and contains a valid `store_id` (UUID v4) and `agent-trace_version` field.

**The `.agent-trace/` directory is excluded from git tracking** (via `.agent-trace/repo/info/exclude`).

### 4.4 Git-Backed Version Control

All version control operations are handled by a self-managed git repository inside `.agent-trace/repo/`. The `git2` crate provides programmatic access — no `git` CLI dependency required.

#### 4.4.1 Repository Setup

On `agent-trace init`, the system:

1. Creates `.agent-trace/repo/` as a bare-like git repository
2. Configures it with `--git-dir=.agent-trace/repo` and `--work-tree=<store-root>`
3. Sets up `.agent-trace/repo/info/exclude` to ignore `.agent-trace/` itself
4. Creates an initial commit with any existing tracked files
5. Creates a `.gitignore` at the store root with default patterns:

```
# agent-trace default ignore patterns
.agent-trace/
*.tmp
*.swp
*.swo
...
| `agent-trace` Operation | Git Mechanism |
|---------------------|---------------|
...
`agent-trace` runs a poll cycle (default: every 1 second) that calls `git2::Repository::statuses()` to detect changes. When changes are found:
...
| Actor | Git Author |
|-------|------------|
| User (no agent active) | `User <user@agent-trace>` |
| Agent (via lock file or `--agent` flag) | `Agent: claude-code <agent@agent-trace>` |
| System (context synthesis, log generation) | `agent-trace <system@agent-trace>` |
| LLM (NL command execution) | `User via LLM <llm@agent-trace>` |
...
Every auto-commit uses a structured message format that `agent-trace` can parse back out for the TUI changelog and CLI commands:

```
[agent-trace] modify plan: prd.md, tasks/api.md
...
- **Line 1 (subject):** `[agent-trace] <action> <type>: <file list>` — action is `create`, `modify`, `delete`, `rename`, `restore`, `reclassify`, `violation`, `context-sync`, `log-sync`. Short file list (truncated with `...` if >3 files).
...
**Parsing:** `agent-trace` parses these commit messages to build the TUI changelog, filter by actor/type/file, and generate `agent-trace log` output. The `git2` crate provides full access to commit messages.

#### 4.4.5 Per-Document Version Numbers

Git uses commit hashes, but users want "prd.md is at v3." `agent-trace` computes per-document version numbers on the fly:
...
This is derived, not stored. It's computed when needed (`agent-trace info`, TUI tree panel, etc.) and cached in memory during a session. The `git log --follow` flag ensures renames are followed — if `spec.md` was renamed to `prd.md`, the version count includes the history from both names.
...
- ~~Snapshot garbage collection (`agent-trace gc`)~~ (git gc, runs automatically)
- ~~Custom ignore file (.agent-trace/ignore)~~ (.gitignore)
- ~~Atomic write-tmp-rename for version data~~ (git commits are atomic)

**What remains custom:** manifest.toml (document types, tags, descriptions), write permission enforcement, agent attribution logic, LLM integration, context synthesis, AGENT-TRACE.md generation, and the TUI.
...
description = "Product requirements document for agent-trace"
agent_name = ""
...
**Write pattern:** Atomic write-tmp-rename (write to `.manifest.toml.tmp`, fsync, rename). The manifest is the only file `agent-trace` manages outside of git. It is also committed into the git repo itself so that it's versioned alongside the documents.

### 4.6 Ignore File

Standard `.gitignore` at the store root. Created on `agent-trace init` with sensible defaults:

```
# agent-trace store — ignore patterns
.agent-trace/
*.tmp
*.swp
*.swo
...
Users edit this file directly. `agent-trace` respects it via git's built-in ignore processing.
...
```bash
agent-trace init /path/to/folder
# Creates .agent-trace/ with config, empty manifest, default ignore file
# If .agent-trace/ already exists, validates and reports store status

agent-trace init /path/to/folder --scan
...
1. **Polling detection** — `agent-trace` detects new `.md` files during its poll cycle. Registers as `scratch` (no LLM) or auto-classifies (LLM available). TUI shows: "New file detected: `plan.md` — classified as `plan`" or "New file detected: `notes.md` — type: `scratch` (use `reclassify notes.md <type>` to change)".
2. **User creates via CLI** — `agent-trace add plan my-plan.md` or via the chat interface.
3. **Agent creates a file** — Agent writes to the folder; `agent-trace` detects it on next poll, registers it. Attribution determined by agent-lock or CLI flag (see Section 7.2).

#### 5.1.3 Document Lifecycle Operations

| Operation | CLI Command | Chat Equivalent |
|-----------|-------------|-----------------|
| Add document | `agent-trace add <type> <path>` | "Add this file as a plan" |
| List documents | `agent-trace ls [--type=<type>]` | "Show me all documents" |
| Show document info | `agent-trace info <path>` | "What is prd.md?" |
| Reclassify | `agent-trace reclassify <path> <new-type>` | "Mark this as a log" |
| Remove from tracking | `agent-trace untrack <path>` | "Stop tracking this file" |
| Delete document | `agent-trace rm <path>` | "Delete the scratch notes" |

#### 5.1.4 Agent Log Synthesis (System-Owned)

Log documents are **exclusively written by `agent-trace`** (see Section 4.2.2). This is the system's independent audit trail. The synthesis process:

When `agent-trace` detects file changes attributed to an agent (via lock file or CLI flag):
...
- When an agent session begins (agent-lock file created or `--agent` flag active), `agent-trace` creates a new log document: `logs/<agent-name>-<session-id>.md` (or `logs/<agent-name>-<timestamp>.md` if no session ID).
- All synthesized entries for that session are appended to this document.
- When the agent session ends (lock file removed or `agent-trace` exits), the log is finalized with a session summary.
...
`agent-trace` uses `git2::Repository::statuses()` to detect changes on every poll cycle (default: 1 second). This internally uses git's stat-based index comparison — the same proven approach git uses for `git status`. Fully cross-platform, zero OS-specific edge cases.
...
**Poll interval:** Configurable in `.agent-trace/config.toml`:
...
In the TUI, the poll runs on a background thread. For non-interactive CLI commands (`agent-trace status`, `agent-trace log`), a single poll runs immediately before the command executes.
...
|---------|---------------|
| `agent-trace log` | `git log` — parse structured commit messages, display chronologically. Filterable by `--type`, `--actor`, `--limit`. |
| `agent-trace log <path>` | `git log --follow <path>` — per-document history including renames. |
| `agent-trace diff <path>` | `git diff HEAD~1 HEAD -- <path>` — diff between current and previous commit touching this file. |
| `agent-trace diff <path> v1 v3` | Find the 1st and 3rd commits touching `<path>`, then `git diff <commit1> <commit3> -- <path>`. |
| `agent-trace show <path> v2` | Find the 2nd commit touching `<path>`, then `git show <commit>:<path>`. |
| `agent-trace restore <path> v2` | Checkout the file at that commit, then create a new commit: "Restored prd.md to v2." |
| `agent-trace status` | `git status` — show untracked `.md` files, modified files, deleted files. |
| `agent-trace repair` | Reconcile manifest with git state — re-scan tracked files, rebuild manifest entries for any files present in git but missing from manifest. |

### 5.3 Functionality 3: LLM-Powered CLI & Chat Interface

#### 5.3.1 Embedded LLM

`agent-trace` embeds a local LLM for three purposes:
...
```toml
# Per-store config (.agent-trace/config.toml)
[llm]
enabled = true                    # Set to false to disable LLM entirely
model_path = ""                   # Empty = use global default

# Global config (~/.config/agent-trace/config.toml)
[llm]
model_path = "~/.local/share/agent-trace/models/qwen2.5-3b-instruct.Q4_K_M.gguf"
context_length = 4096
temperature = 0.2
```

**First-run experience:**
1. `agent-trace` starts normally with LLM features disabled
2. Shows a one-time notice: "No LLM model configured. NL commands, auto-classification, and change summaries are disabled. Run `agent-trace model download` to set up a model."
3. All core features work without the LLM

**Model download:** `agent-trace model download [--size=3b|7b]` downloads from Hugging Face to `~/.local/share/agent-trace/models/`. Shows a progress bar. Requires network access.
...
Commands in the TUI are entered without the `agent-trace` prefix:
...
```
+-----------------------------------------------------------------------+
|  agent-trace v0.1.0 -- my-project (12 docs, 47 versions)     [H]elp [Q]uit |
+----------------------------+------------------------------------------+
...
- Supports command history (up/down arrows), persisted to `.agent-trace/command_history.txt`
...
```
  LLM: not configured (run `agent-trace model download` to enable)
```
...
`agent-trace` uses a **file-convention protocol** for agent integration. There is no HTTP server, no Unix socket, no SDK required. Any agent that can read and write files — which is all of them — can integrate with `agent-trace` with zero setup.

Agents discover the store via a generated index file. They read documents by reading files. They write documents by writing files. `agent-trace` handles everything else: classification, versioning, permission enforcement, log synthesis, and context generation.

This means `agent-trace` works with Claude Code, Codex, Aider, LangChain, CrewAI, AutoGen, custom scripts, and any future agent framework — today, without any integration code.

### 7.2 The `AGENT-TRACE.md` Index File

`agent-trace` generates and maintains a `AGENT-TRACE.md` file at the **root of the document store**. This is the entry point for any agent. It is a human-and-agent-readable index of the entire store.

**`AGENT-TRACE.md` is system-generated and auto-updated.** It is regenerated on every poll cycle that detects changes. Agents and users should not edit it — edits will be overwritten.

**Example content:**

```markdown
# AGENT-TRACE — Agent Document Store Index
> Auto-generated by agent-trace v0.1.0. Do not edit — changes will be overwritten.
...
## Plans
- `prd.md` — Product requirements document for agent-trace (v3, user-created)
...
```bash
# Start agent-trace watching for the agent
agent-trace open ./my-project --agent=claude-code

# In another terminal, start the agent pointed at the same directory
claude-code --project ./my-project
```

The agent reads `AGENT-TRACE.md` as part of its context (it's a markdown file in the project root — most agents will ingest it). It reads `context.md` for project state. It writes plan files. `agent-trace` handles the rest.

**Pattern: LangChain / CrewAI / custom orchestrators**

Orchestrators that manage agent tasks can read `AGENT-TRACE.md` and `manifest.toml` programmatically:

```python
import toml

# Discover store
manifest = toml.load(".agent-trace/manifest.toml")
...
# agent-trace auto-detects and versions it on next poll cycle
```
...
with open(".agent-trace/locks/agent-lock.toml", "w") as f:
    toml.dump(lock, f)

# ... agent does its work ...

# Signal end of session
os.remove(".agent-trace/locks/agent-lock.toml")
```
...
|--------------------|--------------------|
| `AGENT-TRACE.md` (store index) | `.agent-trace/` internals (metadata, snapshots, changelog) |
...
Agents interact with **the folder as a normal project directory.** They don't need to know `agent-trace` exists — they just see well-organized markdown files with a helpful index at the root.
...
|                   OS / Filesystem                  |
|  +---------------------------------------------+  |
|  |  Document Store Directory + .agent-trace/         |  |
|  +---------------------------------------------+  |
+---------------------------------------------------+
```
...
Agents (or their wrappers) create `.agent-trace/locks/agent-lock.toml` before operating:
...
While this file exists, all detected file changes are attributed to the named agent. The agent (or wrapper) deletes the file when done. `agent-trace` removes stale locks (where the PID no longer exists) on startup.

**Mechanism 2: CLI Flag**

```bash
agent-trace open /path/to/store --agent=claude-code
```
...
The manifest is the only custom file `agent-trace` manages outside of git. Writes follow this pattern:

1. Serialize updated manifest to `String`
2. Write to `.agent-trace/.manifest.toml.tmp`
...
```bash
# Store management
agent-trace init <path> [--scan]           # Initialize or open a store
agent-trace status [<path>]                # Show store status or file status
agent-trace repair                         # Reconcile manifest with filesystem

# Document operations
agent-trace add <type> <file>              # Register a document with type
agent-trace ls [--type=<type>] [--json]    # List documents
agent-trace info <file>                    # Show document metadata + version summary
agent-trace reclassify <file> <type>       # Change document type (changes write permissions)
agent-trace untrack <file>                 # Remove from tracking (keep file)
agent-trace rm <file>                      # Soft-delete document

# Write permissions
agent-trace unlock <file> --for=agent --duration=<minutes>   # Temporarily allow agent writes
agent-trace violations [--limit=N]         # Show recent write permission violations

# Context management (system-synthesized)
agent-trace context refresh                # Force re-synthesis of context.md
agent-trace context update "<text>"        # Queue a user update for next synthesis
agent-trace context show                   # Print current context.md to stdout
agent-trace context updates                # Show pending (unincorporated) user updates

# Version control
agent-trace log [<file>] [--limit=N]       # Show changelog
agent-trace diff <file> [v1] [v2]          # Show diff
agent-trace show <file> <version>          # Print historical version to stdout
agent-trace restore <file> <version>       # Restore (creates new commit)

# Batch
agent-trace replace <find> <replace> [--type=<type>] [--dry-run]

# Model management
agent-trace model download [--size=3b|7b]  # Download recommended model
agent-trace model info                     # Show loaded model info
agent-trace model set <path>               # Set model path in global config

# Interactive mode
agent-trace open [<path>] [--agent=<name>] [--ascii]
```
...
### 10.1 Global Config (`~/.config/agent-trace/config.toml`)
...
### 10.2 Per-Store Config (`.agent-trace/config.toml`)
...
- **Concurrent access:** Advisory lock file (`.agent-trace/locks/instance.lock`) prevents two `agent-trace` instances from writing. Second instance opens in read-only mode with a warning.
- **Data integrity:** Git verifies object integrity via SHA-1 hashes. `git fsck` can verify the entire repository.
- **Manifest recovery:** If `manifest.toml` is corrupted, `agent-trace repair` reconstructs it by scanning all `.md` files tracked by git. Document types default to `scratch` (user re-classifies).
...
- `.agent-trace/` directory permissions set to `0700` on creation.
...
- [ ] `agent-trace init` creates a valid store with `.agent-trace/`, config, manifest, git repo, and `.gitignore`
- [ ] `agent-trace init --scan` detects existing `.md` files and registers them as `scratch`
- [ ] `agent-trace open` launches the TUI with all three panels (tree, changelog, chat)
...
- [ ] `agent-trace log`, `agent-trace diff`, `agent-trace show`, `agent-trace restore` all delegate to git operations correctly
- [ ] `agent-trace status` shows untracked files, modifications, and deleted files via `git status`
- [ ] `agent-trace replace` performs batch find-and-replace with preview and confirmation
- [ ] `agent-trace repair` rebuilds manifest from git-tracked files
...
- [ ] `agent-trace unlock` grants temporary write override for protected documents
- [ ] `agent-trace violations` shows recent write permission violations
...
- [ ] `AGENT-TRACE.md` index file is auto-generated at the store root and updated every poll cycle that detects changes
- [ ] `AGENT-TRACE.md` includes: how-to-use instructions for agents, document listing by type, store stats
- [ ] `context.md` is generated on init (minimal template without LLM, synthesized overview with LLM)
- [ ] `agent-trace context refresh` forces a context re-synthesis
- [ ] `agent-trace context update "..."` queues a user update for the next synthesis cycle
...
- [ ] `.agent-trace/ignore` file support with gitignore-style patterns
...
- [ ] `agent-trace model download` downloads a model with progress bar
...
| Manifest corruption | Low | High | Atomic writes. `agent-trace repair` recovery. Snapshot files are independent backup of all content. |
...
- Crash simulation: kill process mid-write -> restart -> `agent-trace repair` recovers
...
| Term | Definition |
|------|------------|
| **Document Store** | A local directory managed by `agent-trace`, containing documents and a `.agent-trace/` metadata directory |
...
| **Actor** | The entity that made a change: user, agent, agent-trace, or llm |
| **Version** | A sequential integer (1, 2, 3...) for each saved state of a document |
| **Store Root** | The top-level directory containing the document store and `.agent-trace/` |
...
| v0.5.0 | Context is now system-synthesized (not user-authored). Added agent integration protocol via `AGENT-TRACE.md` index file. Added `agent-trace context` commands. Added `context_updates.jsonl` for user steering. |
