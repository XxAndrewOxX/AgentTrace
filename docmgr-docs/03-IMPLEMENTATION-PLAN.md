# Implementation Plan: `docmgr` v1.0
...
- Module stubs: `src/config.rs`, `src/manifest.rs`, `src/git_store.rs`, `src/permissions.rs`, `src/commands/mod.rs`, `src/poll.rs`, `src/tui/mod.rs`, `src/llm/mod.rs`, `src/docmgr_md.rs`, `src/context.rs`, `src/log_synth.rs`
...
- `docmgr --help` and `docmgr <subcommand> --help` show correct usage
...
Implement global config (`~/.config/docmgr/config.toml`) and per-store config (`.docmgr/config.toml`) loading with per-store overriding global.
...
struct StoreConfig {
    store: StoreInfo,  // id, name, created, docmgr_version
...
- `StoreInfo.docmgr_version` is the binary's version
...
### Task 1.4: Store Initialization (`docmgr init`)

Create the `.docmgr/` directory structure and all initial files.

**Steps on `docmgr init <path>`:**
1. Check if `.docmgr/config.toml` exists → if yes, validate and report status
2. Create `.docmgr/` directory with mode `0700`
...
5. Create `.docmgr/locks/` directory
6. Create empty `.docmgr/context_updates.jsonl`
7. Create empty `.docmgr/command_history.txt`
...
**Steps on `docmgr init <path>` with existing non-store folder:**
...
**Acceptance tests:**
- Init on empty directory → `.docmgr/` created with all expected files
- Init on non-empty directory without `--scan` → `.docmgr/` created, no files registered
...
**Goal:** A `GitStore` struct that wraps `git2::Repository` and provides all version control operations `docmgr` needs. This is a clean abstraction — the rest of the codebase never touches `git2` directly.

### Task 2.1: Repository Initialization

Create a git repository inside `.docmgr/repo/` configured with the store root as its work tree.
...
**Init steps:**
1. `git2::Repository::init_opts()` with `workdir_path` set to store root and `git_dir` set to `.docmgr/repo`
2. Write `.docmgr/repo/info/exclude` with `.docmgr/` pattern
3. Create initial empty commit: "docmgr store initialized"

**Acceptance tests:**
- After init, `.docmgr/repo/` is a valid git repository
- `repo.workdir()` returns the store root
- `.docmgr/` is excluded from git tracking
...
**Acceptance tests:**
- Commit with actor `User` → git author is `User <user@docmgr>`
- Commit with actor `Agent("claude-code")` → git author is `Agent: claude-code <agent@docmgr>`
- Commit with actor `System` → git author is `docmgr <system@docmgr>`
- Commit message subject matches format: `[docmgr] modify plan: prd.md`
...
- Restore creates a new commit with message `[docmgr] restore plan: prd.md`
...
Violations are recorded as git commits with action `violation` and as entries retrievable by `docmgr violations`.

**Acceptance tests:**
- Agent modifies protected file → violation commit created with structured message
- `docmgr violations` lists the violation with correct details
...
**Goal:** Every `docmgr` CLI command works in non-interactive mode (no TUI). Each command is a function that takes parsed args + dependencies and returns a result.

### Task 5.1: `docmgr status`
...
### Task 5.2: `docmgr add`, `docmgr untrack`, `docmgr rm`
...
### Task 5.3: `docmgr ls`, `docmgr info`
...
### Task 5.5: `docmgr log`, `docmgr diff`, `docmgr show`, `docmgr restore`
...
### Task 5.7: `docmgr unlock`, `docmgr violations`
...
### Task 5.8: `docmgr context` Subcommands
...
### Task 5.10: `docmgr model` Subcommands
...
**Logic:**
1. Check `.docmgr/locks/agent-lock.toml` → if valid, return `Agent(name)`
...
10. Generate/update DOCMGR.md (Task 6.5)
...
- File is committed by system with `docmgr <system@docmgr>` authorship
...
**Acceptance tests:**
- After init with 3 files → `DOCMGR.md` lists all 3 under correct type headings
- Add a new plan → DOCMGR.md updated on next cycle with new file
- Delete a file → DOCMGR.md updated, file no longer listed
- DOCMGR.md includes "Rules" section telling agents what they can/can't modify
- DOCMGR.md committed into git by system

### Task 6.6: Instance Locking

Prevent two `docmgr` instances from writing to the same store.

**Implementation:** Create `.docmgr/locks/instance.lock` with PID on startup. Check for stale locks (dead PID). Second instance opens in read-only mode with warning.
...
**State:** Input buffer, cursor position, command history (loaded from `.docmgr/command_history.txt`).
...
`docmgr model download` fetches a GGUF model from Hugging Face.

**Acceptance tests:**
- Download with `--size=3b` → correct model URL, progress bar displayed, file saved to `~/.local/share/docmgr/models/`
...
1. `docmgr init ./test-project --scan` on a folder with 3 `.md` files
2. Verify: `.docmgr/` created, manifest has 3 entries (type=scratch), git repo initialized with initial commit, `.gitignore` exists, `DOCMGR.md` generated
3. `docmgr open ./test-project`
...
2. `docmgr open ./test-project --agent=claude-code`
3. Externally create a new `.md` file in the store
4. Wait 2 seconds (2 poll cycles)
5. Verify: new file detected, registered as scratch, committed with agent authorship, log entry created in `logs/claude-code-*.md`, DOCMGR.md updated
...
4. `docmgr log doc.md` → shows 3 entries
5. `docmgr diff doc.md v1 v3` → shows diff from "version 1" to "version 3"
6. `docmgr show doc.md v2` → prints "version 2"
7. `docmgr restore doc.md v1` → file on disk is "version 1", log shows 4 entries
8. `docmgr info doc.md` → shows version v4
...
5. `docmgr violations` → shows 1 violation for `ref.md`
6. `docmgr unlock ref.md --for=agent --duration=5` → override created
...
2. `docmgr replace "PostgreSQL" "MySQL" --type=plan`
...
2. `docmgr context refresh` → `context.md` generated (template without LLM)
3. `docmgr context update "We chose PostgreSQL"` → entry in context_updates.jsonl
4. `docmgr context updates` → shows pending update
5. `docmgr context refresh` → context.md includes the update (with LLM mock)
6. Modify a plan document → context re-synthesis triggered

### IT-7: Concurrent Instance Detection

1. Start `docmgr open` (instance A)
2. Start `docmgr open` (instance B, same store)
...
3. Restart `docmgr` → `.tmp` cleaned up, manifest intact
4. `docmgr repair` → manifest consistent with git state
...
| 6 | 6.5 DOCMGR.md Gen | Not Started | |
