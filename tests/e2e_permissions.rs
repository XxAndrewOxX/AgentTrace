...
#[test]
fn pi1_01_user_create_plan() {
    assert_eq!(
        check_permission(&DocType::Plan, &Actor::User, &no_overrides(), None),
        PermissionResult::Allowed
    );
}
// Row 2: User create context → Confirmation (system-synthesized)
#[test]
fn pi1_02_user_create_context() {
    assert!(matches!(
        check_permission(&DocType::Context, &Actor::User, &no_overrides(), None),
        PermissionResult::RequiresConfirmation { .. }
    ));
}
// Row 3: User create log → Confirmation
#[test]
fn pi1_03_user_create_log() {
    assert!(matches!(
        check_permission(&DocType::Log, &Actor::User, &no_overrides(), None),
        PermissionResult::RequiresConfirmation { .. }
    ));
}
// Row 4: User create reference → Allowed
#[test]
fn pi1_04_user_create_reference() {
    assert_eq!(
        check_permission(&DocType::Reference, &Actor::User, &no_overrides(), None),
        PermissionResult::Allowed
    );
}
// Row 5: User create scratch → Allowed
#[test]
fn pi1_05_user_create_scratch() {
    assert_eq!(
        check_permission(&DocType::Scratch, &Actor::User, &no_overrides(), None),
        PermissionResult::Allowed
    );
}
// Row 6: User modify plan → Allowed
#[test]
fn pi1_06_user_modify_plan() {
    assert_eq!(
        check_permission(&DocType::Plan, &Actor::User, &no_overrides(), None),
        PermissionResult::Allowed
    );
...
#[test]
fn pi1_09_user_modify_reference() {
    assert_eq!(
        check_permission(&DocType::Reference, &Actor::User, &no_overrides(), None),
        PermissionResult::Allowed
    );
...
#[test]
fn pi1_14_user_delete_reference() {
    assert_eq!(
        check_permission(&DocType::Reference, &Actor::User, &no_overrides(), None),
        PermissionResult::Allowed
    );
...
#[test]
fn pi1_16_agent_create_plan() {
    assert_eq!(
        check_permission(&DocType::Plan, &agent("x"), &no_overrides(), None),
        PermissionResult::Allowed
    );
}
// Row 17: Agent create context → Denied (agents cannot create context)
#[test]
fn pi1_17_agent_create_context() {
    assert!(matches!(
        check_permission(&DocType::Context, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
}
// Row 18: Agent create log → Denied
#[test]
fn pi1_18_agent_create_log() {
    assert!(matches!(
        check_permission(&DocType::Log, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
}
// Row 19: Agent create reference → Denied
#[test]
fn pi1_19_agent_create_reference() {
    assert!(matches!(
        check_permission(&DocType::Reference, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
}
// Row 20: Agent create scratch → Allowed
#[test]
fn pi1_20_agent_create_scratch() {
    assert_eq!(
        check_permission(&DocType::Scratch, &agent("x"), &no_overrides(), None),
        PermissionResult::Allowed
    );
}
// Row 21: Agent modify plan → Allowed
#[test]
fn pi1_21_agent_modify_plan() {
    assert_eq!(
        check_permission(&DocType::Plan, &agent("x"), &no_overrides(), None),
        PermissionResult::Allowed
    );
...
#[test]
fn pi1_22_agent_modify_context_denied() {
    assert!(matches!(
        check_permission(&DocType::Context, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
...
#[test]
fn pi1_23_agent_modify_log_denied() {
    assert!(matches!(
        check_permission(&DocType::Log, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
...
#[test]
fn pi1_24_agent_modify_reference_denied() {
    assert!(matches!(
        check_permission(&DocType::Reference, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
...
#[test]
fn pi1_25_agent_modify_scratch() {
    assert_eq!(
        check_permission(&DocType::Scratch, &agent("x"), &no_overrides(), None),
        PermissionResult::Allowed
    );
...
#[test]
fn pi1_27_agent_delete_context_denied() {
    assert!(matches!(
        check_permission(&DocType::Context, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
...
#[test]
fn pi1_28_agent_delete_log_denied() {
    assert!(matches!(
        check_permission(&DocType::Log, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
...
#[test]
fn pi1_29_agent_delete_reference_denied() {
    assert!(matches!(
        check_permission(&DocType::Reference, &agent("x"), &no_overrides(), None),
        PermissionResult::Denied { .. }
    ));
...
#[test]
fn pi1_30_agent_delete_scratch() {
    assert_eq!(
        check_permission(&DocType::Scratch, &agent("x"), &no_overrides(), None),
        PermissionResult::Allowed
    );
...
#[test]
fn pi1_31_system_create_context() {
    assert_eq!(
        check_permission(&DocType::Context, &Actor::System, &no_overrides(), None),
        PermissionResult::Allowed
    );
}
// Row 32: System create log → Allowed
#[test]
fn pi1_32_system_create_log() {
    assert_eq!(
        check_permission(&DocType::Log, &Actor::System, &no_overrides(), None),
        PermissionResult::Allowed
    );
}
// Row 33: System modify context → Allowed
#[test]
fn pi1_33_system_modify_context() {
    assert_eq!(
        check_permission(&DocType::Context, &Actor::System, &no_overrides(), None),
        PermissionResult::Allowed
    );
...
fn pi1_24_integration_agent_modify_reference_reverted() {
    ...
    {
        let mut m = manifest.lock().unwrap();
        m.register(&PathBuf::from("ref.md"), DocType::Reference, "")
            .unwrap();
    }
...
fn pi1_23_integration_agent_modify_log_reverted() {
    ...
    {
        let mut m = manifest.lock().unwrap();
        m.register(&PathBuf::from("session.md"), DocType::Log, "")
            .unwrap();
    }
...
fn pi2_override_via_cli() {
    ...
    let out = store
        .run(&["unlock", "api.md", "--for=agent", "--duration=5"])
...
fn pi3_reclassify_changes_enforcement() {
    ...
    {
        let mut m = manifest.lock().unwrap();
        m.register(&PathBuf::from("doc.md"), DocType::Plan, "")
            .unwrap();
    }
    ...
    assert_eq!(
        content, "# v2 by agent",
        "reference modification by agent should be reverted"
    );
}
...
fn pi4_violation_accumulation() {
    ...
    assert!(
        violations.len() >= 3,
        "Expected at least 3 violation entries, got {}",
        violations.len()
    );
}
...
fn pi5_rejected_content_preserved() {
    ...
    assert!(
        !entries.is_empty(),
        "at least one rejected snapshot should exist"
    );
}
