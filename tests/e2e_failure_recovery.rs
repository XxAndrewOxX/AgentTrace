...
fn fr1_manifest_corruption_recovery() {
    ...
    let out = store.docmgr(&["ls"]).expect_success("ls before corrupt");
    ...
    let out = store.docmgr(&["ls"]).expect_success("ls after repair");
...
fn fr2_manifest_deletion_recovery() {
    ...
    // ls should work.
    store.docmgr(&["ls"]).expect_success("ls after manifest deletion repair");
}
...
fn fr3_interrupted_manifest_write_cleanup() {
    ...
    store.docmgr(&["ls"]).expect_success("ls with stale tmp");
...
fn fr6_large_file_tracked() {
    ...
    store.docmgr(&["add", "scratch", "large.md"]).expect_success("add large file");
    ...
    // show v1 works.
    let out = store.docmgr(&["show", "large.md", "1"]).expect_success("show v1");
...
fn fr7_special_characters_in_filenames() {
    ...
    for (name, content) in &files {
        ...
        store.docmgr(&["add", "scratch", name]).expect_success(&format!("add {}", name));
    }
    ...
    store.docmgr(&["info", "my-plan.md"]).expect_success("info my-plan.md");
    store.docmgr(&["info", "my_plan.md"]).expect_success("info my_plan.md");
}
...
fn fr7_file_with_spaces() {
    ...
    store.docmgr(&["add", "scratch", "my plan.md"]).expect_success("add file with space");

    let out = store.docmgr(&["ls"]).expect_success("ls");
    out.assert_stdout_contains("my plan.md");

    store.docmgr(&["info", "my plan.md"]).expect_success("info with space");
}
...
fn fr8_empty_store_operations_no_crash() {
    ...
    let out = store.docmgr(&["ls"]).expect_success("ls empty");
    out.assert_stdout_contains("No documents tracked");

    // log → doesn't crash (may show system init commits)
    store.docmgr(&["log"]).expect_success("log empty");

    // status → "0 document"
    let out = store.docmgr(&["status"]).expect_success("status empty");
    out.assert_stdout_contains("0 document");

    // context refresh → creates minimal context.md
    let out = store.docmgr(&["context", "refresh"]).expect_success("context refresh empty");
    out.assert_stdout_contains("context.md refreshed");

    // violations → "No violations recorded"
    let out = store.docmgr(&["violations"]).expect_success("violations empty");
    out.assert_stdout_contains("No violations recorded");

    // replace → "No matches found"
    let out = store.docmgr(&["replace", "foo", "bar"]).expect_success("replace empty");
